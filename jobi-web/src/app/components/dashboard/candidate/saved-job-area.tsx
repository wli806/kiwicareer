...
const SavedJobArea = ({setIsOpenSidebar}:IProps) => {
  ...
  useEffect(() => {
    const fetchSavedJobs = async () => {
      setLoading(true);
      try {
        const response = await axios.get(`/jobs/wishlist?page=${page}&limit=${itemsPerPage}&sort=${sort}`);
        setSavedJobs(response.data.wishlist);
        setTotalPages(response.data.total_pages);
        setTotalCount(response.data.total_count);

        if (response.data.wishlist.length === 0 && page > 1) {
          setPage(1);
        }
      ...
      <div className="position-relative">
        ...
        {loading ? (
          <div className="text-center py-5">
            <h3 className="text-dark fw-bold">Loading Saved Jobs...</h3>
          </div>
        ) : savedJobs.length === 0 ? (
          <div className="text-center py-5">
            <h3 className="text-dark fw-bold">No saved jobs found</h3>
            <button 
                  ...
                  <div className="row justify-content-between align-items-center">
                    ...
                    <div className="col-lg-2 col-md-4">
                      <div className="action-dots float-end">
                        ...
                        {/* Action Dropdown */}
                        <ActionDropdown jobId={j.id} onDelete={handleDeleteJob} />
                      </div>
                    </div>
                  </div>
                </div>
